...
int ppu_convert_to_rgb(u8 col){
    const u8* rgb = ppu_colors[col];
    return color(rgb[0], rgb[1], rgb[2]);
}

u16 ppu_get_vram_addr(VRAM_ALIGN align, u16 addr, u16 vram_size){
    if(align == VRAM_V){
        bool v11 = addr & (1 << 11);
...
void ppu_get_tile(ppu_t* ppu, u16 base, u8 palette, u8 idx, int tile[64]){
    for(int py = 0; py < 8; py++){
        ...
        for(int px = 0; px < 8; px++){
            ...
            u8 b1 = !!(p1 & (1 << (7 - px)));
            u8 col = b0 | (b1 << 1);
            u8 nes_col = ppu_get_palette_color(ppu, palette, col);
            tile[px + py * 8] = ppu_convert_to_rgb(nes_col);
        }
    }
}
...
void ppu_draw_chr(ppu_t* ppu, SDL_Window** win){
    ...
    for(int y = 0; y < 16; y++){
        for(int x = 0; x < 32; x++){
            int tile[64];
            ppu_get_grayscale_tile(ppu, 0, x + y * 32, tile);
        ...
        for(int x = 0; x < 2; x++){
            ...
            for(int tx = 0; tx < 32; tx++){
                for(int ty = 0; ty < 30; ty++){
                    ...
                    ppu_get_tile(ppu, pattern_base, palette, idx, tile);
...
void ppu_draw_palettes(ppu_t* ppu, SDL_Window** win){
    ...
    for(int p = 0; p < 8; p++){
        for(int c = 0; c < 4; c++){
            u8 nes_col = ppu_get_palette_color(ppu, p, c);
            int col = ppu_convert_to_rgb(nes_col);
            for(int y = 0; y < 16; y++)
                for(int x = 0; x < 16; x++)
...
void ppu_draw_oam(ppu_t* ppu, SDL_Window** win){
    ...
    for(int y = 0; y < 8; y++){
        for(int x = 0; x < 8; x++){
            ...
            int tile[64];
            ppu_get_tile(ppu, base_addr, 0x4 | (sp.attr & 0b11), idx | flip_y, tile);
...
void ppu_inc_addr(ppu_t* ppu){
    ppu->v += ppu->ctrl & (1 << 2) ? 32 : 1;
}

void ppu_sync(ppu_t* ppu){
    for(int i = 0; i < 3; i++)
        ppu_step(ppu);
}
...
void ppu_read_nt(ppu_t* ppu){
    u16 addr = 0x2000 | (ppu->v & 0x0FFF);
    ppu->nt = ppu->read(ppu, addr);
}

void ppu_read_at(ppu_t* ppu){
...
void ppu_read_bg_lsb(ppu_t* ppu){
...
void ppu_read_bg_msb(ppu_t* ppu){
    u16 pattern = ppu->ctrl & (1 << 4) ? 0x1000 : 0x0000;
    u16 fine_y = ppu->v >> 12;
...
void ppu_fill_shifter(ppu_t* ppu){
...
void ppu_put_pixel(ppu_t* ppu){
    ...
    u8 bg_nes_col = ppu_get_palette_color(ppu, bg_pal, bg_col);
    ...
    for(int i = ppu->sprite_count - 1; i >= 0; i--){
        ...
        if(!s->x){
            ...
            if(is_behind && bg_col)
                ...
                final_pix = ppu_get_palette_color(ppu, 0x4 | (s->attr & 0b11), s_col); 
        } else {
            s->x -= 1;
        }
    }

    pixels[screen_x + screen_y * width] = ppu_convert_to_rgb(final_pix);
}

void ppu_load_oam(ppu_t* ppu){
    u32 scanline = ppu->cycles / PPU_CYCLES_PER_SCANLINE;
    ppu->sprite_count = 0;
    ppu->can_hit_0 = false;
    u8 sprite_size = ppu->ctrl & (1 << 5) ? 16 : 8;
...
void ppu_fetch_sprite_lo(ppu_t* ppu){
    u32 intra_scan_cycles = ppu->cycles % PPU_CYCLES_PER_SCANLINE;
...
void ppu_fetch_sprite_hi(ppu_t* ppu){
    u32 intra_scan_cycles = ppu->cycles % PPU_CYCLES_PER_SCANLINE;
...
void ppu_fetch_sprites(ppu_t* ppu){
    u8 sprite_size = ppu->ctrl & (1 << 5) ? 16 : 8;
    for(int i = 0; i < 8; i++){
        sprite_t* s = &ppu->sprites[i];
        u16 base_addr;
